#!/bin/bash -l
#
# This script prepares and executes test for tMSSink through SLURM.
#
...
function show_usage() {
    echo "Usage: $0 [-c LUSTRE_CHUNK] [-d DIRECTORY] [-h] [-n NODES] [-a ANTENNAS] [-C CHANNELS] [-s LUSTRE_STRIPES] [-t TIME] [-w WRITERS]"
...
while getopts :a:C:c:d:hn:s:t:Tw:L option; do
    case "${option}" in
        a) nant=${OPTARG};;
        #b) bucket=${OPTARG};;
...
# number of tasks
let ntasks=${nnode}*${nwriter}

# The number of nodes used for writing
echo "Number of nodes: ${nnode}"
echo "Number of tasks_per_node: ${nwriter}"

# The number of channels per node
let nchannel_per_node=${nchannel}/${ntasks}
...
#
mssink_batch_file="tMSSink_${dir}.sbatch"
...
srun -n ${ntasks} --ntasks-per-node ${nwriter} ${ASKAP_ROOT}/Code/Components/Services/ingest/current/apps/tMSSink.sh -c ${parset_file} > \${log_file}
...
if [[ $testmode -ne 1 ]]; then
    jid1=`sbatch ${mssink_batch_file} | cut -d " " -f 4`
fi
...
post_batch_file="post_${dir}.sbatch"
...
EOF
if [[ $testmode -ne 1 ]]; then
    sbatch --dependency=afterany:${jid1} ${post_batch_file}
fi
