...
{
  const casacore::uInt fftSize = 1024;
  casacore::Vector<casacore::Complex> input(fftSize);
  casacore::Vector<casacore::Complex> result(fftSize,casacore::Complex(0.,0.));
  casacore::FFTServer<casacore::Float,casacore::Complex> server;
  casacore::Vector<casacore::Complex> output(fftSize);
  for (casacore::uInt start = 0; start+fftSize<data.size(); start+=fftSize) {
       for (casacore::uInt i=0; i<fftSize; ++i) {
  ...
  for (casacore::uInt i=0; i<result.nelements(); ++i) {
       os<<i<<" "<<casacore::real(result[i])<<" "<<casacore::imag(result[i])<<std::endl;
  }
}
...
try {
   ...
   casacore::HistAcc<float> histRe(-maxAmp, maxAmp, binWidth);
   casacore::HistAcc<float> histIm(-maxAmp, maxAmp, binWidth);
