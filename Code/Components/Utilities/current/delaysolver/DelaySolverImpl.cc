...
DelaySolverImpl::DelaySolverImpl(double targetRes, casacore::Stokes::StokesTypes pol, float ampCutoff, casacore::uInt refAnt) :
...
void DelaySolverImpl::excludeBaselines(const casacore::Vector<std::pair<casacore::uInt, casacore::uInt> > &bsln) 
{
...
bool DelaySolverImpl::checkAllFlagged(const casacore::Matrix<bool> &flags)
{
...
{
   const casacore::Vector<casacore::Stokes::StokesTypes>& stokes = acc.stokes();
   casacore::uInt pol2use = stokes.nelements();
   for (pol2use = 0; pol2use < stokes.nelements(); ++pol2use) {
        if (stokes[pol2use] == itsPol) {
            break;
        }
   }
   ASKAPCHECK(pol2use < stokes.nelements(), "Unable to find "<<casacore::Stokes::name(itsPol)<<" polarisation product in the data");
   
   const casacore::Matrix<bool>& flags = acc.flag().xyPlane(pol2use);   
   ...
   if (itsFreqAxis.nelements() == 0) {
       ...
       if (itsTargetRes > abs(actualRes)) {
           ...
           itsChanToAverage = casacore::uInt(itsTargetRes / abs(actualRes));
       } 
       ...
       const casacore::uInt targetNChan = acc.nChannel() / itsChanToAverage;
       ...
       itsSpcBuffer.set(casacore::Complex(0.,0.));
       itsAvgCounts.set(0u);
   } else {
   ...
   const casacore::Matrix<casacore::Complex> vis = acc.visibility().xyPlane(pol2use);   
   for (casacore::uInt row = 0; row < acc.nRow(); ++row) {
        const casacore::Vector<casacore::Complex> thisRowVis = vis.row(row);
        const casacore::Vector<bool> thisRowFlags = flags.row(row);
        ASKAPDEBUGASSERT(itsSpcBuffer.nrow() == acc.nRow());
        ASKAPDEBUGASSERT(itsAvgCounts.nrow() == acc.nRow());        
        casacore::Vector<casacore::Complex> thisBufRowVis = itsSpcBuffer.row(row);
        casacore::Vector<casacore::uInt> thisRowCounts = itsAvgCounts.row(row);
        
        const double thisRowApproxDelay = delayApproximation(row);        
        
        ASKAPDEBUGASSERT(thisRowVis.nelements() == thisRowFlags.nelements());
        for (casacore::uInt chan = 0, index = 0; chan < thisBufRowVis.nelements(); ++chan) {
             for (casacore::uInt ch2 = 0; ch2 < itsChanToAverage; ++ch2,++index) {
                  ASKAPDEBUGASSERT(index < thisRowFlags.nelements());
                  
                  if (!thisRowFlags[index]) {
                      const casacore::Complex curVis = thisRowVis[index];
                      if ((itsAmpCutoff < 0) || (abs(curVis) < itsAmpCutoff)) {
                          if (itsDelayApproximation.nelements() > 0) {
                              ...
                              const double phase = -casacore::C::_2pi * (itsFreqAxis[index] - itsFreqAxis[0]) * thisRowApproxDelay;
...
double DelaySolverImpl::delayApproximation(casacore::uInt row) const
{
  if (itsDelayApproximation.nelements() == 0) {
      return 0.;
  }
  ...
  const casacore::uInt ant1 = itsAnt1IDs[row];
  const casacore::uInt ant2 = itsAnt2IDs[row];
...
casacore::Vector<double> DelaySolverImpl::solve(bool useFFT) const
{
  ...
  const casacore::uInt nAnt = casacore::max(casacore::max(itsAnt1IDs), casacore::max(itsAnt2IDs)) + 1;
  ...
  std::set<casacore::uInt> rows2exclude;
  ASKAPDEBUGASSERT(itsAnt1IDs.nelements() == itsAnt2IDs.nelements());
  ASKAPDEBUGASSERT(itsAnt1IDs.nelements() == itsAvgCounts.nrow());
  for (casacore::uInt row=0; row<itsAnt1IDs.nelements(); ++row) {
       // first, exclude explicitly listed baselines
       for (casacore::uInt bsln = 0; bsln < itsExcludedBaselines.nelements(); ++bsln) {
       ...
       if (rows2exclude.find(row) == rows2exclude.end()) {
           const casacore::Vector<casacore::uInt> thisRowCounts = itsAvgCounts.row(row);
           bool allFlagged = true;
           for (casacore::uInt chan = 0; chan < thisRowCounts.nelements(); ++chan) {
  ...
  std::set<casacore::uInt> excludedAntennas;
  for (casacore::uInt ant = 0; ant < nAnt; ++ant) {
       bool dataPresent = false;
       bool referencePresent = false;
       for (casacore::uInt bsln=0; bsln<itsAnt1IDs.nelements(); ++bsln) {
  ...
  casacore::Vector<double> delays(itsSpcBuffer.nrow()+1,0.);
  casacore::Vector<double> quality(itsSpcBuffer.nrow(),0.);
  casacore::Matrix<double> dm(delays.nelements(),nAnt,0.);
  std::ofstream os("avgspectrum.dat");
  for (casacore::uInt bsln = 0; bsln < itsSpcBuffer.nrow(); ++bsln) {
       if (rows2exclude.find(bsln) == rows2exclude.end()) {
           casacore::Vector<casacore::Complex> buf = itsSpcBuffer.row(bsln).copy();
           const casacore::Vector<casacore::uInt> thisRowCounts = itsAvgCounts.row(bsln);
           ASKAPDEBUGASSERT(buf.nelements() == thisRowCounts.nelements());           
           for (casacore::uInt chan=0; chan < buf.nelements(); ++chan) {
                if (thisRowCounts[chan] > 0) {
                ...
                os<<itsAnt1IDs[bsln]<<" "<<itsAnt2IDs[bsln]<<" "<<chan<<" "<<arg(buf[chan])/casacore::C::pi*180.<<std::endl;
           }
           ...
           const casacore::uInt ant1 = itsAnt1IDs[bsln];
           ASKAPDEBUGASSERT(ant1 < dm.ncolumn()); 
           const casacore::uInt ant2 = itsAnt2IDs[bsln];
  ...
  if (excludedAntennas.size() == 0) {
      ...
      for (std::set<casacore::uInt>::const_iterator rowIt = rows2exclude.begin(), antIt = excludedAntennas.begin(); antIt != excludedAntennas.end(); ++antIt) {
  ...
  const casacore::Matrix<double> dmt = transpose(dm);
  const casacore::Matrix<double> nm = product(dmt,dm);
  
  casacore::Vector<double> result = product(invert(nm), product(dmt,delays));
...
/// @param[in] delays
void DelaySolverImpl::setApproximateDelays(const casacore::Vector<double> &delays)
{
  if (delays.nelements()) {
