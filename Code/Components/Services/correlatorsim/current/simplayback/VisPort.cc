...
void VisPort::send(const askap::cp::VisDatagramBETA& payload)
{
    boost::system::error_code error;
    itsSocket.send(boost::asio::buffer(&payload, sizeof(VisDatagramBETA)), 0, error);
    if (error) {
        ASKAPLOG_ERROR_STR(logger, "UDP send failed: " << error);
    }
}

void VisPort::send(const std::vector<askap::cp::VisDatagramBETA>& payload)
{
