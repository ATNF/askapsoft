...
class Permutation 
{
	public :
		...
		uint32_t const getTotal(const uint32_t nItem) const;
		...
		uint32_t const getIndex(const uint32_t item1, 
				const uint32_t item2) const;
		
		/// Given the items, return the permutation index. 
		/// @param[in] items    Item pair, where element 1 <= element 2
		/// @return Permutation index		
		uint32_t const getIndex(const std::pair<uint32_t,uint32_t> items) const;
		...
		/// @param[in] nItem    Total number of items
		/// @return Permutation index		
		uint32_t const getIndex(const std::pair<uint32_t,uint32_t> 
				items, const uint32_t nItem) const;
		...
		std::pair<uint32_t,uint32_t> const getItems
