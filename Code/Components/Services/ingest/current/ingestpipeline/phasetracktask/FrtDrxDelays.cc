...
void FrtDrxDelays::process(const askap::cp::common::VisChunk::ShPtr& chunk, 
                           const casacore::Matrix<double> &delays,
                           const casacore::Matrix<double> &/*rates*/,
                           const double effLO)
{
  ...
  for (casacore::uInt ant = 0; ant < delays.nrow(); ++ant) {
       ...
       casacore::Int drxDelay = static_cast<casacore::Int>(2048. + diffDelay);
       if (drxDelay < 0) {
  ...
  for (casacore::uInt row = 0; row < chunk->nRow(); ++row) {
       ...
       if (itsFrtComm.isValid(ant1) && itsFrtComm.isValid(ant2)) {
           ...
           if (itsTrackResidualDelay) {
              ...
              const float phase = 2. * static_cast<float>(casacore::C::pi * effLO * appliedDelay);
              const casacore::Complex phasor(cos(phase), sin(phase));
