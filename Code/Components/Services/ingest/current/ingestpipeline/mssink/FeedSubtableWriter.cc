...
void FeedSubtableWriter::defineOffsets(const casacore::Matrix<casacore::Double> &offsets)
{
   ...
   if (itsUpdateCounter == 0u) {
       ...
       if (offsets.ncolumn() != itsOffsets.ncolumn()) {
           ...
           for (casacore::Matrix<casacore::Double>::const_contiter ci1 = offsets.cbegin(), ci2 = itsOffsets.cbegin(); ci1 != offsets.cend(); ++ci1,++ci2) {
                ASKAPDEBUGASSERT(ci2 != itsOffsets.cend());
                if (casacore::abs(*ci1 - *ci2) > offsetTolerance()) {
                    itsOffsetsChanged = true;
                    break;
                }
           }
...
{
   ASKAPCHECK(!itsOffsetsChanged, "Attempted to set the new beam offsets while the previous ones were not written yet");
   const casacore::uInt nBeams = feedCfg.nFeeds();
   if (itsUpdateCounter == 0u) {
       // initialise buffer
   ...
   for (casacore::uInt beam = 0; beam < nBeams; ++beam) {
        ...
        if (!itsOffsetsChanged) {
            if (itsUpdateCounter > 0u) {
                ...
                if (casacore::abs(itsOffsets(0, beam) - x) > offsetTolerance() || casacore::abs(itsOffsets(1, beam) - y) > offsetTolerance()) {
                    itsOffsetsChanged = true;
                }
            } else {
                itsOffsetsChanged = true;
...
void FeedSubtableWriter::write(casacore::MeasurementSet &ms, double time, double interval)
{
   ...
   if (itsUpdateCounter > 0) {
       ...
       for (casacore::uInt row = itsStartRowForLastUpdate; row < startRow; ++row) {
   ...
   const casacore::uInt nBeams = itsOffsets.ncolumn();
   ASKAPDEBUGASSERT(itsOffsets.nrow() == 2u);
   ms.feed().addRow(nBeams * itsNumberOfAntennas);

   for (casacore::uInt ant = 0, row = startRow; ant < static_cast<casacore::uInt>(itsNumberOfAntennas); ++ant) {
        for (casacore::uInt beam = 0; beam < nBeams; ++beam,++row) {
              ...
              // Feed position
              const casacore::Vector<double> feedXYZ(3, 0.0);
