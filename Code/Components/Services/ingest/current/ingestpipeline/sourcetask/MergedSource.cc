...
VisChunk::ShPtr MergedSource::next(void)
{
    ...
    for (int32_t count = 0; (!itsMetadata ||  itsMetadata->scanId() == ScanManager::SCANID_IDLE) && 
        ...
        if (!itsMetadata && itsVisConverter.config().receiverId() >= 0) {
            ASKAPLOG_DEBUG_STR(logger, "Received empty metadata object for stream "<<itsVisConverter.config().receiverId()<<", attempt number "<<(count + 1)<<
                                       " out of "<<maxNoMetadataRetries);
        }
    } 
    ...
    while (itsMetadata->time() != itsVis->timestamp) {
        ...
        if (itsMetadata->time() > itsVis->timestamp) {
            ...
            ASKAPCHECK((itsMetadata->time() - itsVis->timestamp) < ONE_SECOND * 120, 
                     "Junk BATs suspected. More than two minute discrepancy between metadata and visibility timestamps for stream id="<<itsVisConverter.config().receiverId()<<
                     ", metadata time: "<<bat2epoch(itsMetadata->time()).getValue()<<
                                           " visibility time: "<<bat2epoch(itsVis->timestamp).getValue()<<
                     " resync/reprogram of the correlator usually helps to recover until proper cross-checks are done by the IOC");
            //
