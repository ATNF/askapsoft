...
if [ ${BUILD_MODEL_FOR_CONTSUB} == true ]; then
...
if [ "${DIRECTION_SCI}" != "" ]; then
    ...
    freq=\`grep -A1 Ch1 \$log | tail -n 1 | awk '{print \$8 + \$9*\$6/2./1000.}'\`
fi
...
aprun -n 2 -N 2 ${cmodel} -c \${parset} > \${log}
err=\$?
NUM_CPUS=1
extractStats \${log} \${SLURM_JOB_ID} \${err} cmodel_contsub_spectral_B${BEAM} "txt,csv"
if [ \$err != 0 ]; then
    exit \$err
fi
