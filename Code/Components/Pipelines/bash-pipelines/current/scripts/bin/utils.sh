...
function setContPreimagingDeps()
{
    CONT_PREIMAGE_DEPS=""
    CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$DEP_START")
    CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$ID_SPLIT_SCI_LIST")
    CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$ID_CCALAPPLY_SCI_LIST")
    CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$ID_FLAG_SCI_LIST")
    CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$ID_AVERAGE_SCI_LIST")
    CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$ID_FLAG_SCI_AV_LIST")
    if [ "${DO_SPLIT_TIMEWISE}" == "true" ]; then 
        CONT_PREIMAGE_DEPS=$(addDep "$CONT_PREIMAGE_DEPS" "$ID_MSCONCAT_SCI_AV")
    fi
}

function setSpectralPreimagingDeps()
{
    SPECTRAL_PREIMAGE_DEPS=""
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$DEP_START")
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_SPLIT_SCI_LIST")
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_FLAG_SCI_LIST")
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_CCALAPPLY_SCI_LIST")
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_SPLIT_SL_SCI_LIST")
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_CAL_APPLY_SL_SCI_LIST")
    SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_CONT_SUB_SL_SCI_LIST")
    if [ "${DO_SPLIT_TIMEWISE}" == "true" ]; then 
        SPECTRAL_PREIMAGE_DEPS=$(addDep "$SPECTRAL_PREIMAGE_DEPS" "$ID_MSCONCAT_SCI_SPECTRAL")
    fi
}
