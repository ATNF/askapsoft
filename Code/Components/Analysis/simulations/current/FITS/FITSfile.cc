...
void FITSfile::convolveWithBeam()
{
    if (!itsHaveBeam) {
        ...
        int specdim = (itsWCS->spec > 0) ? itsAxes[itsWCS->spec] : 1;
        int stokesdim = getNumStokes();
        for (int z = 0; z < specdim; z++) {
            ASKAPLOG_DEBUG_STR(logger, "Smoothing channel " << z << " out of a total of " << specdim);
            for (int j = 0; j < stokesdim; j++) {
                ...
                for (size_t pix = 0; pix < xySize; pix++) {
                    ...
                    if (getNumStokes() == 1) {
                        ...
                        if (getStokesAxis() > itsWCS->spec) {
                            loc += z * xySize + j * specdim * xySize;
                        } else {
                            loc += j * xySize + z * stokesdim * xySize;
                        }
                    }
                    image[pix] = itsArray[loc];
                }
                ...
                for (size_t pix = 0; pix < xySize; pix++) {
                    ...
                    itsArray[loc] = newArray.get()[pix] / scaleFactor;
                }
            }
        }
